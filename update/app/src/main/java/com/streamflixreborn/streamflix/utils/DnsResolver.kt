...
object DnsResolver : Dns {
    ...
    private const val BYPASS_TAG = "Cine24hBypass"
    ...
    override fun lookup(hostname: String): List<InetAddress> {
        ...
        // Log con tag unico per monitorare Cine24h
        if (hostname.contains("cine24h")) {
            Log.d(BYPASS_TAG, "[DNS] Resolving host: $hostname using provider: $providerName")
        } else {
        ...
        return try {
            ...
            if (hostname.contains("cine24h")) {
                Log.d(BYPASS_TAG, "[DNS] Resolved $hostname to: ${addresses.joinToString { it.hostAddress ?: "" }}")
            }
            addresses
        } catch (e: Exception) {
